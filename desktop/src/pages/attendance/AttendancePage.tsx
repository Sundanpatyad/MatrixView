...
export function AttendancePage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <h2 className="mt-10 text-sm font-semibold tracking-wide text-ink-500 uppercase">
        Recent days
      </h2>
      <div className="mt-3 space-y-2">
        {history.map((row) => (
          ...
          >
            <div>
              <p className="text-sm font-semibold text-ink-900">{row.date}</p>
              <p className="text-xs text-ink-500">
                {row.in} – {row.out} · Break {row.breakMins}m
              </p>
            </div>
            <span className="text-xs font-semibold text-ink-600">{row.note}</span>
          </div>
        ))}
      </div>
    </div>
