...
| Backend | Node.js modular monolith, MongoDB (Mongoose), Redis/Socket.IO/BullMQ planned in-process | Folder structure `scaffolded` |
...
- Desktop UI (browser): `cd desktop && npm install && npm run dev` → http://localhost:5175
...
## C2b. Desktop UI checklist (employee workspace)

Employee client — Jira-style project → board → issues flow. Own attendance/activity only.

| Route | Screen | Folder / file | Status |
|---|---|---|---|
| `/login` | First-run login → lands on Projects | `desktop/src/pages/auth/LoginPage.tsx` | `designed` |
| `/projects` | Create project first (required), then list & open boards | `desktop/src/pages/projects/ProjectsPage.tsx` | `designed` |
| `/projects/:id/board` | Full-screen Jira-like board: drag/drop columns, create Task/Bug/Story/Time, click → detail modal (creator, assignee, estimate/logged time) | `desktop/src/pages/board/ProjectBoardPage.tsx` | `designed` |
| `/` | Today: check-in + shortcuts to project boards | `desktop/src/pages/today/TodayPage.tsx` | `designed` |
| `/attendance` | Check in/out, break, recent days | `desktop/src/pages/attendance/AttendancePage.tsx` | `designed` |
| `/activity` | Own activity timeline | `desktop/src/pages/activity/ActivityPage.tsx` | `designed` |
| `/notifications` | In-app notification feed | `desktop/src/pages/notifications/NotificationsPage.tsx` | `designed` |
| `/profile` | Own profile edit | `desktop/src/pages/profile/ProfilePage.tsx` | `designed` |

**Desktop board infra:** `WorkspaceContext` (projects/tasks in localStorage), `BoardTaskCard`, `CreateTaskModal`, `TaskDetailModal`. Vite port 5175. Tauri 2 (`npm run tauri:dev`); Rust tracking/sync still stubbed.

## C3. Next implementation slices

1. Wire web + desktop auth to backend Auth APIs (replace mock contexts).
2. Scaffold Node/Express (or Fastify) + Mongoose in `backend/` for Auth → Org → Employees → Attendance → Tasks.
3. Wrap desktop React UI in Tauri; implement heartbeat / idle / secure refresh-token storage in Rust.
4. Replace mock tables with live API data; add real Kanban drag-and-drop.
